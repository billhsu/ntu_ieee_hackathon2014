...
s.linkInternalFilters="javascript:,shopbop.com"
...
s.visitorNamespace="amazonshopbop"
s.trackingServer="amazonshopbop.122.2o7.net"
...
function s_doPlugins(s) {
	...
	if (s_account=="amznshopbopprod") {
		s.trackExternalLinks=true
		s.linkInternalFilters="javascript:,shopbop.com"
	}

	/* Add calls to plugins here */
	//DTM - overwrite pageName if DTM got one
	if (_satellite.getVar('DDpageName')) { s.pageName=_satellite.getVar('DDpageName'); }
	// default pageName to URL if not defined
	if (!s.pageName) { s.pageName=location.protocol + "//" + location.host + location.pathname; }
	
	if (window.s_prePlugins) { s_prePlugins(s) }
	
	// process internal search
	// blank prop3 & prop5 as needed, set event2
	if (s.prop2) {
		s.prop2=s.prop2.toLowerCase()
		s.eVar2=s.prop2
		if (s.prop3=="pageName") {s.prop3=s.pageName}
		temp=s.getValOnce(s.prop2,"s_p2",0)
		if(temp){s.events=s.events?s.events+",event2":"event2"}
	} else { s.prop3=""; s.prop5="" }
	
	// set eVar11 based on prop11
	s.eVar11=s.getValOnce(s.prop11,"s_v11",0)
	
	// set eVar5 from prop4
	s.eVar5=s.getValOnce(s.prop4,"s_v5",0)
	
	// set eVar3 based on purchaseID or eVar13
	if (!s.eVar3) { 
		if (s.purchaseID) { s.eVar3=s.purchaseID; }
		if (s.eVar13) {s.eVar3=s.eVar13; }
	}
	
	// set prop1 to channel if not already populated
	if (!s.prop1) { s.prop1=s.channel }
	
	// clear products if no events set
	if (!s.events) { s.products="" }
	
	// set automatic cart open
	s.events=s.getCartOpen("s_scOpen");
	
	// trigger new visit flag for VISTA
	s.prop50=s.getVisitFlag("s_visit");
...
* https://omniture-help.custhelp.com/app/answers/detail/a_id/1422/kw/plugins
*/
...
/* Plugin: Visit Flag v1.0 */
s.getVisitFlag=new Function("c",""
+ "var s=this,v=0,t=new Date;"
+ "t.setTime(t.getTime()+1800000);"
+ "if (s.c_r(c)) { s.c_w(c,1,t) }"
+ "else { if (s.c_w(c,1,t)) {	v=1; }}"
+ "return v;");
